...
async def _call_gemini(user_prompt: str) -> str:
    ...
    # 2. Define the config once so it stays identical across all 3 models
    ...
    # 3. The 3-Tier Model Waterfall
    try:
        # ATTEMPT 1: Primary Model (3.1)
        ...
        logger.warning(f"[Formatter] 3.1-flash-lite failed: {e1}. Routing to 3.5...")
        
        try:
            # ATTEMPT 2: First Fallback (3.5)
            response = await client.aio.models.generate_content(
                model="gemini-3.5-flash-lite",
                contents=user_prompt,
                config=shared_config,
            )
            logger.info("[Formatter] ✅ Fallback to 3.5-flash-lite succeeded.")
            reply = response.text.strip()
            
        except Exception as e2:
            logger.warning(f"[Formatter] 3.5-flash-lite failed: {e2}. Routing to 3.8...")
            
            try:
                # ATTEMPT 3: Final Fallback (3.8)
                response = await client.aio.models.generate_content(
                    model="gemini-3.8-flash",
                    contents=user_prompt,
                    config=shared_config,
                )
                logger.info("[Formatter] ✅ Fallback to 3.8-flash succeeded.")
                reply = response.text.strip()
                
            except Exception as e3:
                # ALL ATTEMPTS FAILED - Raise error to trigger the credit rollback in main.py
                logger.error(f"[Formatter] ❌ All models failed (3.1 -> 3.5 -> 3.8). Last error: {e3}")
                raise e3 

    # 4. 🚀 META API 4096 CHAR LIMIT FAILSAFE
    if len(reply) > 4000:
